...
import {COOKIES, STATUS_CODE} from "../../../common/constant.js";
...
export const signInController = async function (context) {
    ...
    if (!body.password || !body.phoneNumber) {
        throw new Api_error("Nous n'acceptons pas de champs vide", "login fail", STATUS_CODE.BAD_REQUEST)
    }
    ...
    if (!user || !isUser) {
        throw new Api_error('Le mots de passe ou le numéro de telephone est incorrecte😒.', "login fail", STATUS_CODE.UNAUTHORIZED)
    }
    // create cookie
    await createCookie({id: user._id}, context)
